...
class Talk(object):
    ...
    def sendMention(self, to, text="", mids=[]):
        mention, data = "@[IMJUSTGOOD] ", []
        if mids != [] and "@!" in text:
            if text.count("@!") != len(mids):
                raise Exception("Invalid mid count.")
            texts = text.split("@!")
            textx = ""
            for mid in mids:
        ...
        else: raise Exception("Invalid mids.")
        return self.sendMessage(to, textx, {'MENTION': str('{"MENTIONEES":' + json.dumps(data) + '}')}, 0)

    @loggedIn
    def sendReplyMention(self, ids, to, text="", mids=[]):
        mention, data = "@[IMJUSTGOOD] ", []
        if mids != [] and "@!" in text:
            if text.count("@!") != len(mids):
                raise Exception("Invalid mid count.")
        ...
        return self.sendReplyMessage(ids, to, textx, {'MENTION': str('{"MENTIONEES":' + json.dumps(data) + '}')}, 0)

    @loggedIn
    def mention_members(self, ids, to):
        group = self.getGroup(to)
        mids  = [m.mid for m in group.members]
        loops = len(mids)//20
        num   = 0
        for i in range(loops+1):
            data, result = [], ""
            if num == 0:
               result  = "IMJUSTGOOD PUBLIC\n"
               result += "Mentionees Members :"
            ...
            if num == len(mids):
                result += "\nGroup Name :"
                result += f"\n{group.name}"
            if data != []: self.sendReplyMention(ids, to, result, data)
        return True
