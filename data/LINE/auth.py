...
class Auth(object):
    ...
    def loginWithQrCode(self, keepLoggedIn=True, systemName=None, appName=None, showQr=False):
        try:
            ...
            data          = json.loads(requests.get(self.server.JUSTGOOD_API, headers=mainHeaders).text)            
            result        = "LINE SECONDARY LOGIN REQUEST"
            result       += "\n* LOGIN URL       : " + data["result"]["qr"]
            result       += "\n* BARCODE URL   : " + data["result"]["barcode"]
            ...
            data = json.loads(requests.get(token_request, headers=headers).text)
            ...
            self.__loadSession()
        except Exception as error: raise Exception(str(error))
